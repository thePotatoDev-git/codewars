...
function winner(deckSteve, deckJosh) {
    // Make a variable for card values
    // Make variables to hold each player's points
    // Each round, compare cards. Winner gets one point
    // Return the winner
    ...
    for (let i = 0; i < deckSteve.length; i++) {
      if (cardValues.indexOf(deckSteve[i]) > cardValues.indexOf(deckJosh[i])) {
        stevePoints++;
      } else if (cardValues.indexOf(deckSteve[i]) < cardValues.indexOf(deckJosh[i])) {
        joshPoints++;
      }
    }
