...
function findRoutes(routes) {
    // Find the first route by finding which city doesn't have a second occurrence
    // Find the currentRoute by filtering routes to include the last city, but not the previous city
    // Keep looping until currentRoute is empty
  
    // Find first route
    const routesJoined = routes.join(' ');
    let start = '';
  
    for (let i = 0; i < routes.length; i++) {
      if (routesJoined.indexOf(routes[i][0]) === routesJoined.lastIndexOf(routes[i][0])) {
        start = routes[i][0];
      }
    }
  
    const destinations = routes.filter(arr => arr.includes(start))[0];
    let end = false;
    
    while (end === false) {
      let currentRoute = routes.filter(arr => arr.includes(destinations[destinations.length - 1]) && !arr.includes(destinations[destinations.length - 2]))
  
      if (currentRoute.length > 0) {
        destinations.push(currentRoute[0][1]);
      } else {
        end = true;
      }
