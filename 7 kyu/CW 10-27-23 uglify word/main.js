...
function uglifyWord(s) {
    // Make a "flag" variable starting at 1
    // Make a variable for the alphabet
    // Lowercase s then split s
    // Loop through s
    // If s[i] is an abc char and flag is 1, uppercase the letter. Then change flag to 0
    // If s[i] is an abc char and flag is 0, lowercase the letter. Then change flag to 1
    // If s[i] is not an abc char, change flag to 1
    ...
    for (let i = 0; i < sArr.length; i++) {
      if (abc.includes(sArr[i]) && flag === 1) {
        sArr[i] = sArr[i].toUpperCase();
        flag = 0;
      } else if (abc.includes(sArr[i]) && flag === 0) {
        sArr[i] = sArr[i].toLowerCase();
        flag = 1;
      } else if (!abc.includes(sArr[i])) {
        flag = 1;
      }
    }
