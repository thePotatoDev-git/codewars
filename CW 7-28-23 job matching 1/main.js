...
function match(candidate, job) {
    // If a salary is missing, throw an error
    // Subtract 10% from candidate's min salary
    // If job's max salary is >= to candidate's min salary, return true. Else return false.
    
    if (candidate.minSalary === undefined || job.maxSalary === undefined) {
      throw error;
    }
    
    const wiggleMin = candidate.minSalary * 0.9;
    
    return job.maxSalary >= wiggleMin;
}